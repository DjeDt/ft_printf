/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   do_long.c                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ddinaut <marvin@42.fr>                     +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/11/10 17:34:34 by ddinaut           #+#    #+#             */
/*   Updated: 2017/11/30 19:00:42 by ddinaut          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "printf.h"

char	*add_exeption(void *to_add, t_opt opt, long long int i)
{
	if (check_int_exception(i, opt) == 1)
		to_add = ft_strjoin_fr(" ", to_add);
	else if (check_int_exception(i, opt) == 2)
		to_add = ft_strjoin_fr("+", to_add);
	return (to_add);
}

int		do_long(va_list arg, t_opt opt, char c, void **final)
{
	int				ret;
	long long int	i;
	void			*to_add;

	to_add = NULL;
	i = va_arg(arg, long long int);
	if (c == 'D')
		to_add = convert_int(i, 10);//ft_itoa_base(i, 10);
	else if (c == 'O')
		to_add = convert_int(i, 8);//ft_itoa_base(i, 8);
	else if (c == 'U')
		to_add = convert_int(i, 10);//ft_itoa_base(i, 10);
	if (check_int_exception(i, opt) == 1)
		to_add = ft_strjoin_fr(" ", to_add);
	else if (check_int_exception(i, opt) == 2)
		to_add = ft_strjoin_fr("+", to_add);
	ret = concat_to_str(final, to_add, opt);
	return (ret);
}
